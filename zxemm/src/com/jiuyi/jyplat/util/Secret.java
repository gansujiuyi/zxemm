 ...
 * @author          	 xuf
 * @version	 		     V1.0
 * @see		      		 com.blueder.bcms.util.Secret
 *
 ********************************************************************************************
...
public class Secret {
	//加密
	public static Logger Log = Logger.getLogger(Secret.class);
	...
	//加密 用固定密钥
	public static byte[] doEncrypt(byte[] plainText) throws Exception {
		byte[] key = { 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11 };

		//return doEncrypt ( "z=a~qv!2".getBytes() , plainText ) ;
		byte[] endPlainText = new byte[plainText.length + 1];
		for (int i = 0; i < plainText.length; i++) {
			endPlainText[i] = plainText[i];
		}

		endPlainText[plainText.length] = 0x00;

		//return doEncrypt ( key , plainText ) ;
		return doEncrypt(key, endPlainText);
	}

	//解密
	public static byte[] doDecrypt(byte[] desKey, byte[] encryptText) throws Exception {
		//      DES算法要求有一个可信任的随机数源
		SecureRandom sr = new SecureRandom();
	...
	*/
	public static String encrypt(String src_txt) throws Exception {
