			...
			<form id=PageForm name=PageForm method=post action="">
				<div id="itemList">
					<div class="loc">
						...
						<a href="javascript:gotoListHome()" ><span class="last">资金监管资金解冻</span></a> 
					</div>
					<!-- 全/反选 -->
							...
							<table width="100%" border="0">
								...
								<td width="5%" >房产证号：</td>
				              	...
				              	<tr>
									...
									<td width="5%" >
										<a href="javascript:submitForm('capitalManage/queryUnfreezeCapitalInfoAll.do')">
							...
							<table class="mainTab" id="itemListTab">
								...
								<c:forEach var="capital" items="${pageFinder.data}">
									<tr >
										...
										<td class="thStyle">
											<c:choose>
												<c:when test="${capital.orderStatus eq '1010'}">
													订单刚创建
												</c:when>
												<c:when test="${capital.orderStatus eq '1011'}">
													卖方填写完毕
												</c:when>
												<c:when test="${capital.orderStatus eq '1020'}">
													等待支付
												</c:when>
												<c:when test="${capital.orderStatus eq '1030'}">
													未支付完成
												</c:when>
												<c:when test="${capital.orderStatus eq '1031'}">
													支付完成
												</c:when>
												<c:when test="${capital.orderStatus eq '1032'}">
													银行确认
												</c:when>
												<c:when test="${capital.orderStatus eq '1040'}">
													过户成功，等待扣款
												</c:when>
												<c:when test="${capital.orderStatus eq '1050'}">
													打款完成
												</c:when>
												<c:when test="${capital.orderStatus eq '1110'}">
													行方已经确定需退款
												</c:when>
												<c:when test="${capital.orderStatus eq '1111'}">
													退款中，已写退款流水，等待划款动作完成
												</c:when>
												<c:when test="${capital.orderStatus eq '1112'}">
													退款完成
												</c:when>
											</c:choose>	
										</td>
...
<form id="capitalByIdInfo" action="capitalManage/queryUnfreezeCapitalByIdInfo.do" method="post">
