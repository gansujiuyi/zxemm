...
function modifyState(orderId,state){
	$("#orderId").val(orderId);
	$("#state").val(state);
	if(confirm("确定要冻结笔业务吗？") == false){
			return;
		}
	$("#capitalStateF").submit();
}

function snyHouseInfo(orderId){
	$("#orderId").val(orderId);
	$("#isSny").val("1");
	$("#capitalStateF").attr("action", "capitalManage/queryCapitalByIdInfo.do");
	$("#capitalStateF").submit();
}

function saveDKrecordDK(orderId,url,opeType){
	$("#orderId").val(orderId);
	$("#opeType").val(opeType);
	
	document.capitalStateF.action = url;
	if(confirm("确定要经行打款操作吗？") == false){
			return;
		}
...
function saveDKrecordQX(orderId,url,opeType){
	$("#orderId").val(orderId);
	$("#opeType").val(opeType);
	
	document.capitalStateF.action = url;
	if(confirm("确定取消该笔业务吗？") == false){
			return;
		}
	document.capitalStateF.submit();
}
				...
				<!-- 提交按钮 -->
					<div style="padding: 10px; padding-left: 10px;">
						<div style="width: 250px; float: left;">
							...
							<c:if test="${orderinfo.orderExStatus eq '0000' && orderinfo.orderStatus ne '1033' && orderinfo.orderStatus ne '1034'}">
								<a href="javascript:modifyState('${orderinfo.orderId}','1111')" class="a-bluebtn">
									<img src="<%=path%>/images/template/b_disable.png" alt="IMG" />冻结
								</a><!-- 如果是正常状态就显示冻结 -->
							</c:if>
							<c:if test="${orderinfo.orderStatus eq '1034' && orderinfo.priceTotal == cashPayTotal}">
								<a href="javascript:saveDKrecordDK('${orderinfo.orderId}','<%=path%>/capitalManage/saveDKrecord.do','1')" class="a-bluebtn">
										<img src="<%=path%>/images/template/b_enable.png" alt="IMG" />打款
								</a>
							</c:if>
							<c:if test="${orderinfo.orderStatus eq '0004' && orderinfo.isRefunded eq '0000' }">
								<a href="javascript:saveDKrecordTK('${orderinfo.orderId}','<%=path%>/capitalManage/saveDKrecord.do','2')" class="a-bluebtn">
										<img src="<%=path%>/images/template/b_disable.png" alt="IMG" />退款
								</a>
							</c:if>
							<c:if test="${orderinfo.orderStatus eq '1010' || orderinfo.orderStatus eq '1011' || orderinfo.orderStatus eq '1012' || orderinfo.orderStatus eq '1020'}">
							<c:if test="${orderinfo.isRefunded ne '1113'}">
								<a href="javascript:saveDKrecordQX('${orderinfo.orderId}','<%=path%>/capitalManage/saveDKrecord.do','2')" class="a-bluebtn">
										<img src="<%=path%>/images/template/b_disable.png" alt="IMG" />取消
								</a>
								</c:if>
							</c:if>
							<c:if test="${orderinfo.orderStatus eq '1020' && orderinfo.isRefunded eq '0000' }">
							    <c:if test="${ empty payInfo}">
									<a href="/emm/mehouseorder/delOrderInfo.do?orderId=${orderinfo.orderId}" class="a-bluebtn">
											<img src="<%=path%>/images/template/b_disable.png" alt="IMG" />删除
									</a>
								</c:if>
							</c:if>
								<a href="javascript:snyHouseInfo('${orderinfo.orderId}');" class="a-bluebtn">
										<img src="<%=path%>/images/template/b_enable.png" alt="IMG" />同步过户信息
								</a>
						<div class="clr"></div>
					</div>
			</form>
...
<form id="capitalStateF" name="capitalStateF" action="capitalManage/modifyCapitalByIdInfo.do" method="post">
	...
	<input id="isSny" type="hidden" name="isSny"/>
</form>
		</div>
	</body>
