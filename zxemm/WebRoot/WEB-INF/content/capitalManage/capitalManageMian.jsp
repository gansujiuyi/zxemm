			...
			<form id=PageForm name=PageForm method=post action="">
				<div id="itemList">
					<div class="loc">
						...
						<a href="javascript:gotoListHome()" ><span class="last">资金监管管理查询</span></a> 
					</div>
					<!-- 全/反选 -->
						...
						<div style="float: left" class="search">
							<table width="100%" border="0">
								<tr>
									...
									<td width="5%" >监管流水号：</td>
									<td colspan="3" ><input id="" name="orderinfo.orderId" value="${orderinfo.orderId}" type="text" class="txt-150" />
									...
									<select name="orderinfo.orderStatus">
										...
										<option value="1032" <c:if test="${orderinfo.orderStatus eq '1032'}">selected="selected"</c:if>>银行确认，等待房产局确认过户状态</option>
										...
										<option value="1041" <c:if test="${orderinfo.orderStatus eq '1041'}">selected="selected"</c:if>>过户成功，等待打款</option>
										...
										<a href="javascript:submitForm('capitalManage/queryCapitalInfoAll.do')">
				              	...
				              	<td width="5%" >房源编号：</td>
									<td>
	                                	<input id="" name="orderinfo.sstr1" value="${orderinfo.sstr1}" type="text" class="txt-150" /></td>
					               <td width="5%" >创建时间：</td>
									<td >
							...
							<table class="mainTab" id="itemListTab">
								...
								<c:forEach var="capital" items="${pageFinder.data}">
									<tr >
										...
										<td class="thStyle">
											<c:choose>
												<c:when test="${capital.orderStatus eq '1010' && capital.isRefunded eq '0000'}">
													订单刚创建
												</c:when>
												<c:when test="${capital.orderStatus eq '1011' && capital.isRefunded eq '0000'}">
													卖方填写完毕
												</c:when>
												<c:when test="${capital.orderStatus eq '1012' && capital.isRefunded eq '0000'}">
													双方信息填写完毕
												</c:when>
												<c:when test="${capital.orderStatus eq '1020' && capital.isRefunded eq '0000' && empty capital.lstr9}">
													等待支付
												</c:when>
												<c:when test="${capital.orderStatus eq '1020' && capital.isRefunded eq '0000' && capital.lstr9 eq '1020'}">
													等待支付
												</c:when>
												
												<c:when test="${capital.orderStatus eq '0004' && capital.isRefunded eq '0000'}">
													支付完成
												</c:when>
												
												<c:when test="${capital.orderStatus eq '1020' && capital.isRefunded eq '0000' && capital.lstr9 eq '0003'}">
													首付款已支付
												</c:when>
												
												<c:when test="${capital.orderStatus eq '1032' && capital.isRefunded eq '0000'}">
													银行确认，等待房产局确认过户状态
												</c:when> 
												<c:when test="${capital.orderStatus eq '1033' && capital.isRefunded eq '0000'}">
													房产所开始过户
												</c:when>
												<c:when test="${capital.orderStatus eq '1034' && capital.isRefunded eq '0000'}">
													房产所过户完成
												</c:when>
												<c:when test="${capital.orderStatus eq '1035' && capital.isRefunded eq '0000'}">
													房产所过户失败
												</c:when>
												<c:when test="${capital.orderStatus eq '1040' && capital.isRefunded eq '0000'}">
													过户成功，等待扣款
												</c:when>
												<c:when test="${capital.orderStatus eq '1041' && capital.isRefunded eq '0000'}">
													过户成功，等待打款
												</c:when>
												<c:when test="${capital.orderStatus eq '1042' && capital.isRefunded eq '0000'}">
													确认打款，等待审核
												</c:when>
												<c:when test="${capital.orderStatus eq '1050' && capital.isRefunded eq '0000'}">
													打款完成
												</c:when>
												<c:when test="${capital.isRefunded eq '1113'}">
													取消，撤销订单
												</c:when>
												<c:when test="${capital.isRefunded eq '1110'}">
													行方已经确定需退款
												</c:when>
												<c:when test="${capital.isRefunded eq '1111'}">
													退款中，已写退款流水，等待划款动作完成
												</c:when>
												<c:when test="${capital.isRefunded eq '1112'}">
													退款完成
												</c:when>
										...
										<td class="thStyle" style="text-align:left;">
											<a href="javascript:searchCapitalByIdInfo('${capital.orderId}')">
											<img src="<%=path%>/images/template/b_search.png" alt="IMG" />查看详情</a>
											<c:if test="${capital.orderStatus eq '1034' && capital.isRefunded eq '0000'}">
											<!-- <a href="javascript:javascript:searchCapitalByIdInfo('${capital.orderId}')" class="a-bluebtn">
													<img src="<%=path%>/images/template/b_enable.png" alt="IMG" />打款
											</a> -->
											</c:if>
											<c:if test="${capital.orderStatus eq '0004' && capital.isRefunded eq '0000'}">
											<a href="javascript:javascript:searchCapitalByIdInfo('${capital.orderId}')" class="a-bluebtn">
												<img src="<%=path%>/images/template/b_disable.png" alt="IMG" />退款
											</a>
											</c:if>
										</td>
									 </tr>
...
<form id="capitalByIdInfo" action="capitalManage/queryCapitalByIdInfo.do" method="post">
