...
function submitForm(url, act)
{
    if(act == "add")
    {
    	var operNo = document.getElementById("operNo");
		...
		var wxBranchName=document.getElementById("wxBranchName");
		
		if(!checkFormdata(operNo,"操作员编号",10,true,true,true,false,false))
		{	
			return false;
		}
				...
				<div class="mainCon">
					...
					<table class="editorTb" style="" id="tbBase" name="editorTab">
						<tr>
							<td class="hd" colspan="2">
								新建系统操作员（<font color="red">* 为必填项</font>）
							</td>
						</tr>
						<tr>
							<td class="titl">
							...
							<td>
								<input id="operNo" name="oper.operNo" type="text" maxlength="10" class="txt-150"/>
								<font color="red">&nbsp;*</font>
							</td>
						</tr>
						<tr>
							...
							<td>
								<input id="operName" name="oper.operName" type="text" maxlength="10" class="txt-150"/>
								<font color="red">&nbsp;*</font>
							</td>
						</tr>
						<tr>
							...
							<td>
								<input id="operMobile" name="oper.operMobile" type="text" maxlength="20" class="txt-150"/>
								<font color="red">&nbsp;*</font>
							</td>
						</tr>
						<tr>
							...
							<td>
								<select id="roleId" name="oper.role.roleId" class="sel-250">
				              		...
				              		<c:forEach var="roleT" items="${roles}">
										<option value="${roleT.roleId }">${roleT.roleName }-${roleT.description }</option>
									</c:forEach>
				              	</select>
				              	<font color="red">&nbsp;*</font>
									...
									<c:forEach var="inst" items="${insts}">									
											...
											<c:forEach var="inst2" items="${insts}">
												<c:if test="${inst2.parentInstitutionId==inst.institutionId}">
													<option value="${inst2.institutionId }">
													...
													<c:forEach var="inst3" items="${insts}">
														<c:if test="${inst3.parentInstitutionId==inst2.institutionId}">
															<option value="${inst3.institutionId }">
						...
						<tr>
							...
							<td>
								<input type="hidden" class ="sel-300" id="wxBranchId" name="oper.wxBranchId"/>
				              	<font color="red">&nbsp;*</font>当用户为网点操作员时请选择正确的银行网点，按网点名中文搜索，只显示前40条。
								<input type="hidden" id="wxBranchName" name="oper.wxBranchName"/>
							</td>
						</tr>
						<tr>
							<td class="titl">
								...
								<select id="department" name="oper.department" class="sel-250">
				              		...
				              		<c:forEach var="enum" items="${sysEnum.sysEnumItems}">
										<c:if test="${enum.status=='1' }">
											<option value="${enum.fieldValue }">${enum.displayValue }</option>
										</c:if>
									</c:forEach>
				              	</select>
				              	<font color="red">&nbsp;*</font>
						...
						<tr>
							...
							<td>
								<select id="operLvl" name="oper.operLvl" class="sel-250">
				              		...
				              		<option value="1">行员级</option>
				              		<option value="2">机构级</option>
				              		<option value="3">管理员级</option>
				              		<!-- <option value="4">全辖级</option> -->
				              	</select>
				              	<font color="red">&nbsp;*</font>
							</td>
						</tr>
					...
					<div class="btn-wz">
						...
						<input type="button" name="btnSave" value=""
							onclick="submitForm('acegi/saveAddedOper.do','add')"
