					...
					<div class="loc">
						...
						<div id="PrintContent" class="mainCon">
							<table class="mainTab" id="itemListTab">
								...
								<c:if test="${empty refundPayInfos }">
									<tr><td colspan=8 align="center" height="25"><font color="red">没有查询到相关记录</font></td></tr>
								</c:if>
								<c:forEach var="contactRefPay" items="${refundPayInfos}"
									varStatus="status">
									<tr>
										<td>
											${contactRefPay.contactNo }
										</td>
										<td>
											<c:if test="${contactRefPay.payStatus == '0000'}">未支付</c:if>
											<c:if test="${contactRefPay.payStatus == '0010'}">支付完成</c:if>
											<c:if test="${contactRefPay.payStatus == '2222'}">退款完成</c:if>
										</td>
										<td>
											${contactRefPay.payMoney }元
										</td>
										<td>
											${contactRefPay.createTime}
										</td>
										<td>
											${contactRefPay.payDay }
										</td>
										<td>
											${contactRefPay.refundTime }
										</td>
										<td>
											${contactRefPay.payTradeNo }
										</td>
										<td>
											${contactRefPay.oppName }
										</td>
									</tr>
								</c:forEach>
							</table>
