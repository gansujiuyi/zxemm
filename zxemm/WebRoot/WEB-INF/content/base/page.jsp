...
function queryPage(page){
	...
	-->
    	 总计<span style='color:red;margin:0 2px'>${pageFinder.rowCount}</span>条记录  共<span style='color:red;margin:0 2px'>${pageFinder.pageCount}</span>页 每页<span style='color:red;margin:0 2px'>${pageFinder.pageSize}</span>条 &nbsp;&nbsp;
    	<input name="bp" id="bp" type="button" <c:if test="${!pageFinder.hasPrevious}">disabled="disabled"</c:if> class="page_button_first" value="第一页" onClick="queryPage(1)"/> 
    	<input name="bp" id="bp" type="button" <c:if test="${!pageFinder.hasPrevious}">disabled="disabled"</c:if> class="page_button_prev" value="上一页" onClick="queryPage(${pageFinder.pageNo-1})"/> 
    	<input name="bp" id="bp" type="button" <c:if test="${!pageFinder.hasNext}">disabled="disabled"</c:if> class="page_button_next" value="下一页" onClick="queryPage(${pageFinder.pageNo+1})"/> 
    	<input name="bp" id="bp" type="button" <c:if test="${!pageFinder.hasNext}">disabled="disabled"</c:if> class="page_button_last" value="最末页" onClick="queryPage(${pageFinder.pageCount})"/>&nbsp;&nbsp;
    	<input id="query.page" name="query.page" type="text" maxlength="8" value="${pageFinder.pageNo}" size="3" onkeyup="value=value.replace(/[^\d]/g,'')"/>&nbsp;&nbsp;
