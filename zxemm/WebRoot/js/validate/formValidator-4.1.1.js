...
$.formValidator = 
{
	...
	setTipState : function(elem,showclass,showmsg)
	{
		...
		if(showmsg==null || showmsg=="")
		{
			jqobj = $(elem);
			if($(elem).attr("class")){
				var oldClass = $(elem).attr("class").replace("input_focus","").replace("input_error","").replace("input_correct","");
				jqobj.removeClass().addClass(oldClass);
				tip.hide();
			}
		}
		else
		{
			...
			else
			{
				...
				if(html.length = 0 || showmsg == ""  || showmsg == "输入正确" || showmsg == "请输入")
				{
					tip.hide();
				}
				else
...
var formValidator_setting = 
{
	validatorGroup : "1",
	onShowText : "",
	onShowTextColor:{mouseOnColor:"#333",mouseOutColor:"#999999"},
	onShowFixText:"",
	onShow :"请输入内容",
	onFocus: "请输入内容",
	onCorrect: "输入正确",
